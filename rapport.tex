...
Pour expliquer en clair ce qu'est l'apprentissage supervisé dans un langage plus souple, nous prenons un échantillon de l'ensemble de données \guillemotleft California Housing Prices  \guillemotright\footnote{L'ensemble de données original est apparu dans R. Kelley Pace et Ronald Barry, “Sparse Spatial Autoregressions,” Statistics \& Probability Letters 33, no. 3 (1997): 291–297} qui contient des informations sur un ensemble de districts à Californie aux États-Unis, nous choisissons seulement cinq (5) enregistrements de cette base de données juste pour illustrer notre exemple, comme on peut remarquer à la table \ref{tab:CaliforniaHousingPrices}. Le but est de prédire le prix moyen du logement dans un district de Californie tout en 



%Après une analyse prédictive, nous avons extrait les cinq attributs les plus pertinents pour notre démarche qui sont : le code du produit, la description du produit, le type du produit, la date d'expiration et la quantité vendue. Un problème d'apprentissage supervisé serait de prédire, au moment de passer une commande, la quantité d'un produit qui pourrait être vendue au mois prochain. Souvent les personnels responsables des inventaires aimeraient avoir une idée sur les quantités à commander. Nous considérons que cette base de données contient des exemples de produits avec leurs quantités vendues au cours du mois actuel où chaque produit est lié à une quantité vendue. \\      
...
\begin{table}[h]
	...
	\label{tab:CaliforniaHousingPrices}
\end{table}

Dans cet exemple, l'ensemble des variables prédictives est le vecteur défini de la façon suivante : $\vec{X} = \begin{bmatrix}x_1, x_2, x_3, x_4  \end{bmatrix}$ où $x_1$ représente le code, $x_2$ la description, $x_3$ le type et $x_4$ la date d'expiration. La variable cible est la quantité vendue, c'est celle qu'on veut prédire. La matrice  $A_{5,4}$ est définie comme suit: \\
$A_{5,4} =  
 \begin{pmatrix} 
 	JO0000386 & Jus d'orange & Boissons & 25/03/2019 \\ 
 	PT0000986 & Pâte de tomates & Cuisine & 14/12/2018 \\ 
 	\end{pmatrix} 
$
